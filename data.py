...
class Dataset:
    ...
    def fit(self, cfg):
        ...
        for year in range(begin_year, end_year + 1):
            ...
            if not os.path.exists(PATH + file_name_forcing):
                latitude, longitude = self._load_forcing_or_land_surface(
                    data_path,
                    self.forcing_list,
                    self.s_resolution,
                    self.s_namedict,
        ...
        lat_file_name = "lat_{s}.npy".format(s=self.s_resolution)
        lon_file_name = "lon_{s}.npy".format(s=self.s_resolution)
        ...
        if cfg["memmap"]:
            land_surface = np.memmap(PATH + "land_surface_memmap.npy", dtype=cfg["data_type"], mode="w+", shape=(np.sum(day_list, axis=0), land_surface_list[0].shape[1], land_surface_list[0].shape[2], land_surface_list[0].shape[3]))
        ...
        land_surface = np.memmap(
            PATH + "land_surface_memmap.npy",
            dtype=cfg["data_type"],
            mode="r",
            shape=(
                np.sum(day_list, axis=0),
                land_surface_list[0].shape[1],
                land_surface_list[0].shape[2],
                land_surface_list[0].shape[3],
            ),
        )
        ...
        label = []
        for year in range(begin_year, end_year + 1):
            file_name_label = "ERA5_LAND_label_{sr}_{year}.npy".format(
                sr=self.s_resolution, year=year
            )
            if not os.path.exists(PATH + file_name_label):
                lat, lon = self._load_forcing_or_land_surface(
                    data_path,
                    self.label,
                    self.s_resolution,
                    self.s_namedict,
        ...
        print("label type is {d_p}".format(d_p=label.dtype))
        file_name_mask = "Mask with {sr} spatial resolution.npy".format(
        ...
        print("begin:{begin_year}, end:{end_year}".format(begin_year=self.selected_year[0], end_year=self.selected_year[1]))
        ...
        # get shape
        self.time_length_f, self.nlat_f, self.nlon_f, self.num_features_f = (
            forcing.shape
        )
        self.time_length_l, self.nlat_l, self.nlon_l, self.num_features_l = (
            land_surface.shape
        )

        # np.savetxt("/data/test/forcing.csv",forcing[-1,:,100,:],delimiter=",")
        # N = 365 * len(self.test_year)+self.seq_len
        N = 365 * len(self.test_year) + cfg["seq_len"] + cfg["forecast_time"]

        print("{n} samples for training, {m} samples for testing".format(n=self.time_length_f - N, m=N))
        print("[ATAI {d_p} work ] preprocessing for generating train and test data".format(d_p=cfg["workname"]))
        print("\033[1;31m%s\033[0m" % "The following process are all time-consuming, especially for the high-resolution data. Please wait patiently")
        ...
        if not os.path.exists(PATH + "x_train.npy"):
            x_train = np.memmap(
                ...
                shape=(
                    ...
                    self.nlat_f,
                    self.nlon_f,
                    self.num_features_f + self.num_features_l,
                ),
            )
        ...
        print(
            "[ATAI {d_p} work ] finish  generating y_train data, y_train shape is {yts}".format(
                d_p=cfg["workname"], yts=y_train.shape
            )
        )
        ...
        print(
            "[ATAI {d_p} work ] finish  generating x_test data, x_test shape is {xts}".format(
                d_p=cfg["workname"], xts=x_test.shape
            )
        )
        y_test = label[self.time_length_f - N:]
        print(
            "[ATAI {d_p} work ] finish  generating y_test data, y_test shape is {yts}".format(
                d_p=cfg["workname"], yts=y_train.shape
            )
        )
        ...
        if self.normalize:
            if not os.path.exists(PATH + "x_train_norm.npy") or not os.path.exists(PATH + "y_train_norm.npy"):
                print(
                    "ATAI {d_p} work ] start {nt} normalization forcing".format(
                        d_p=cfg["workname"], nt=cfg["normalize_type"]
                    )
                )
                if cfg["normalize_type"] in ["region"]:
                    ...
                    for i in range(x_train.shape[2]):
                        out_x, out_y = self._get_minmax_scaler(
                            x_train[:, :, i, :],
                            y_train[:, :, i, :],
                            scaler_x[:, :, i, :],
                            scaler_y[:, :, i, :],
                    ...
                    print(
                        "processed: x_train shape is: {x_s}, y_train shape is: {y_s}, x_test shape is: {x_ts_s}".format(
                            x_s=x_train.shape, y_s=y_train.shape, x_ts_s=x_test.shape
                        )
                    )

                    for i in range(x_train.shape[1]):
                        print("[{d_p}-th ] finish".format(d_p=i))
                        out_x_train = self._normalize(
                            x_train[:, i, :, :], "input", scaler_x[:, i, :, :], "minmax"
                        )
                        x_train[:, i, :, :] = out_x_train
                        out_y_train = self._normalize(
                            y_train[:, i, :, :],
                            "output",
                            scaler_y[:, i, :, :],
                            "minmax",
                        ...
                        out_x_test = self._normalize(
                            x_test[:, i, :, :], "input", scaler_x[:, i, :, :], "minmax"
                        )
                        x_test[:, i, :, :] = out_x_test
                    ...
                    scaler_y = np.memmap(
                        ...
                        dtype=cfg["data_type"],
                        mode="w+",
                        shape=(2, y_train.shape[3]),
                    )
                    scaler_y_t = {}
                    for i in range(x_train.shape[3]):
                        out_x, _ = self._get_minmax_scaler(
                            x_train[:, :, :, i],
                            x_train[:, :, :, i],
                            scaler_x[:, i],
                            scaler_y_t,
                    ...
                    for i in range(y_train.shape[3]):
                        out_y, _ = self._get_minmax_scaler(
                            y_train[:, :, :, i],
                            y_train[:, :, :, i],
                            scaler_y[:, i],
                            scaler_y_t,
                    ...
                    for i in range(y_train.shape[3]):
                        ...
                        print("scaler_y_in shape is", scaler_y_in.shape)
                        out_y_train = self._normalize(
                            y_train[:, :, :, i], "output", scaler_y_in, "minmax"
                        )
                        y_train[:, :, :, i] = out_y_train
                    ...
                    for i in range(x_train.shape[3]):
                        ...
                        print("scaler_x_in shape is", scaler_x_in.shape)
                        out_x_train = self._normalize(
                            x_train[:, :, :, i], "input", scaler_x_in, "minmax"
                        )
                        x_train[:, :, :, i] = out_x_train
                        out_x_test = self._normalize(
                            x_test[:, :, :, i], "input", scaler_x_in, "minmax"
                        )
                        x_test[:, :, :, i] = out_x_test
            ...
            x_train_norm = np.memmap(
                PATH + "x_train_norm.npy",
                dtype=cfg["data_type"],
                mode="w+",
                shape=(x_train.shape),
            )
            ...
            x_test_norm = np.memmap(
                PATH + "x_test_norm.npy",
                dtype=cfg["data_type"],
                mode="w+",
                shape=(x_test.shape),
            )
    ...
    def _load_forcing_or_land_surface(
        self,
        root,
        _list,
        s_resolution,
        ...
        print(
            "ATAI {d_p} work ] finish loading {v_n} in {y} year ".format(
                d_p=cfg["workname"], v_n=_list, y=year
            )
        )
    ...
    def reverse_normalize(
        self, feature, variable: str, scaler, scaler_method: str, is_multivars: int
    ) -> np.ndarray:
        ...
        if is_multivars != -1:
            a, b = (
                a[:, :, is_multivars: is_multivars + 1],
                b[:, :, is_multivars: is_multivars + 1],
            )
            c, d = (
                c[:, :, is_multivars: is_multivars + 1],
                d[:, :, is_multivars: is_multivars + 1],
            )
        if variable == "input":
