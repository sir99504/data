...
import numpy as np
import torch
def reverse_normalize(data, scaler, norm_type="minmax"):
    ...
    scaler = np.squeeze(scaler)  
    if scaler.shape[0] != 2:
        raise ValueError("Scaler must have first dimension 2 (min/max)")
    ...
    if isinstance(min_val, np.ndarray):
        ...
        max_val[mask] = min_val[mask] + 1e-8  
        reversed_data = data * (max_val - min_val) + min_val
    else:
...
def r2_score(y_true, y_pred):
    ...
    mask = np.logical_and(np.isfinite(y_true), np.isfinite(y_pred))
